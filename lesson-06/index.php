...
<!-- - - - - - - - - - - - - - - - - - - - - - -
     EXERCISE: THUMBNAIL EXPAND TO IMAGE
     - - - - - - - - - - - - - - - - - - - - - - -->
 <div class="accordion-group">
  <div class="accordion-heading">
...
<!-- - - - - - - - - - - - - - - - - - - - - - -
     EXERCISE: RESERVATION FORM
     - - - - - - - - - - - - - - - - - - - - - - -->
 <div class="accordion-group">
  <div class="accordion-heading">
   <a class="accordion-toggle" data-toggle="collapse" data-parent="#accord_exer" href="#reservForm">
     PHP/AJAX: Reservation Form Exercise
   </a>
  </div>
  <div id="reservForm" class="accordion-body collapse">
   <div class="accordion-inner">
    <div id="rsvrForm" class="row-fluid">
     <div class="form_box span6">
       <h4>The Dew Drop Inn</h4>
       <h5>Make your reservation at our beautiful inn-by-the-lake!</h5>
       <form class="form-horizontal">
         <div class="control-group">
            <label class="control-label" for="clientname">Your Name</label>
            <div class="controls">
              <input type="text" id="clientname" placeholder="Firstname Lastname">
            </div>
         </div>
         <div class="control-group">
            <label class="control-label" for="clientphone">Phone Number</label>
            <div class="controls">
              <input type="tel" id="clientphone" placeholder="nnn-nnn-nnnn">
            </div>
         </div>
         <div class="control-group">
...
<?php	// Set Up Select Fields
	// Define Arrays
	$months = array ('Select Month', 'January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December' );
	$days = range ( 1, 31 );
?>
		<div class="selectgrp row-fluid">  <!-- was form-inline -->
        	<label class="span2">Arrival:</label>
            <select id="arv_mon" class="span4">
            <?php
                $i = 0;
            ...
            <select id="arv_day" class="span3">
              <option value="">Day</option>
            <?php   // Prefill Day Select Options using RANGE()
            ...
            <select id="arv_year" class="span3">
              <option value="">Select Year</option>
            <?php   // Prefill Day Select Options using FOR Statement
                for ($year = 2012; $year <= 2020; $year += 1) {
         ...
         <div class="selectgrp row-fluid">
        	<label class="span2">Departure:</label>
            <select id="dep_mon" class="span4">
            <?php
                $i = 0;
                foreach ($months as $value) {
                    echo "<option value=$i>$value</option>";
            ...
            <select id="dep_year" class="span3">
              <option value="">Select Year</option>
            <?php   // Prefill Day Select Options using FOR Statement
                for ($year = 2012; $year <= 2020; $year += 1) {
                    echo "<option value=$year>$year</option>";
         ...
         <div id="requests" class="row">
           <h5 class="span12">Please select preferences for your stay:</h5>
           <div class="span12">
            <label class="checkbox  span4">Romance Special<input type="checkbox" class="requests" value="romance"></label>
            <label class="checkbox  span4">Spa Special<input type="checkbox" class="requests" value="spa"></label>
            <label class="checkbox  span4">Premium Bathroom<input type="checkbox" class="requests" value="prembath"></label>
           </div>
         </div>

         <button class="btn btn-primary" id="send-reserv">Send</button>

       </form>
     
     </div>   <!--  END DIV rsvrForm span6 -->
     <div class="notes span6">
	   <h4>Some notes about this Exercise:</h4>   
       <ol>
         <li>This exercise explores using a PHP handler to package and send an e-mail based on user inputs. Javascript is used to capture the "SEND" button press event, gather the input values, and pack them into an array sent to the PHP andler via AJAX. </li>
         <li>Select/options were used for the Arrival and Departure dates. I added intelligence via Javascript to populate the <code>option</code> statements in the DOM dynamically:
         <ul>
         	<li>Configure the "Days" drop-down with a number of days based on the month selected,</li>
            <li>Configure the "Years" drop-down to show only Leap Years if Feb. 29 is selected,</li>
            <li>Configure possible Departure Years so that a departure date can't be selected that's before the arrival,</li>
            <li>If Arrival is changed after departure is set, to be after the Departure date, then a warning message is displayed,</li>
            <li>Calculates and displays the number of nights during the stay,</li>
          </ul>
          <li>The PHP handler was written to display as HTML, by using appropriate headers.</li>
          <li>A sample received email can be seen here: <a href="extras/reservation_email.html">Sample Email</a></li>
       </ol>  
     </div>   <!--  END DIV notes span6 -->
    </div>   <!-- END DIV #rsvrForm.row-fluid -->
   </div>   <!-- END DIV accordian-inner -->
  </div>  <!-- END DIV #reservForm.accordian_body.collapse -->

 </div>  <!-- END reservForm accordian_group --> 
...
<script src="js/send_reserv.js"></script>
