...
if ($albums = $mysqli->query("select idalbums, album, release_year, img_file from albums where artists_idartists = ".$artist->idartists)) {
			   while ($album = $albums->fetch_object())
			   {
				 $musicarray[$genre->idgenres]->artists[$artist->idartists]->albums[$album->idalbums] = $album;
				 if ($tracks = $mysqli->query(
				 		"select * from tracks 
				 		where albums_idalbums = " . $album->idalbums . " 
						order by track_num"
						)) {
